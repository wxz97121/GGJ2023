...
public class LanguageSources : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("铁血社区", 0.5f, 0.9f, 0.7f, -1f, 0.1f);
        effect.AddTags("军事", "仇恨");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("儒家经典汇编", 0.1f, 0.1f, 0.4f, 0.3f, 0.8f);
        effect.AddTags("国学");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("中国经典古文集", 0.1f, 0.1f, 0.4f, 0.8f, -1f);
        effect.AddTags("国学");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("外国戏剧赏析", 0.4f, 0.1f, 0.4f, 0.8f, -1f);
        effect.AddTags("外国经典");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("世界文学史", 0.2f, -1f, 0.7f, 0.8f, -1f);
        effect.AddTags("外国经典");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("唐诗宋词汇编", -1f, -1f, 0.8f, 0.9f, -1f);
        effect.AddTags("古诗", "国学");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("考试必背-金牌笔杆子汇总", 0.1f, -1f, 0.1f, 0.3f, -1f);
        effect.AddTags("政治");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("外交辞令必知必会", 0.1f, 0.7f, -1f, -1f, -1f);
        effect.AddTags("政治");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("大陆法系全讲", 0.1f, 0.1f, -1f, -1f, 0.6f);
        effect.AddTags("法律");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("庭审现场", 0.1f, 0.1f, -1f, -1f, 0.6f);
        effect.AddTags("法律");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("法治进程-口述历史记录", 0.1f, 0.1f, -1f, -1f, 0.6f);
        effect.AddTags("法律");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("中东历史全讲", 0.2f, 0.7f, -1f, -1f, -1f);
        effect.AddTags("仇恨", "军事");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("维基解密资料收录", 0.2f, 0.8f, -1f, -1f, 0.1f);
        effect.AddTags("仇恨", "军事");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("生化武器大全", -1f, 1f, -1f, -1f, 0.1f);
        effect.AddTags("仇恨", "军事");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("各国军事纪律手册", -1f, -1f, -1f, 0.1f, 0.1f);
        effect.AddTags( "军事");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("心理操纵发展研究", -1f, -1f, -1f, 0.1f, 0.1f);
        effect.AddTags("心理");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("从中古战争到世界大战资料收集", 0.2f, 0.7f, -1f, -1f, 0.1f);
        effect.AddTags("仇恨", "军事");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("商业帝国演讲全收录", 0.2f, 0.6f, 0.3f, 0.6f, 0.2f);
        effect.AddTags("欺骗");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("革命歌曲收录全集", 0.1f, 0.6f, 0.7f, -1f, 0.7f);
        effect.AddTags("政治", "音乐");
        AllLsEffects.Add(effect);
        ...
        effect.AddTags("色情");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("知乎", 0.75f, 0.8f, 0.5f, 0.6f, 0.4f);
        effect.AddTags("常识", "段子","嘲讽", "职场");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("脉脉社区爬虫数据集", 0.6f, -1f, -1f, -1f, -1f);
        effect.AddTags("职场");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("领英社区爬虫数据集", 0.6f, -1f, -1f, -1f, -1f);
        effect.AddTags("职场");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("Github", 0.2f, -1f, 0.2f, 0.8f, -1f);
        effect.AddTags("科技", "代码");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("微博要闻榜", 0.3f, 0.4f, 0.3f, -1f, -1f);
        effect.AddTags("政治", "情感", "正能量");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("微博文娱榜", 0.7f, 0.8f, 0.8f, -1f, 0.5f);
        effect.AddTags("娱乐", "影视", "情感", "仇恨");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("Bilibili 体育区弹幕库", 0.5f, 0.7f, 0.4f, -1f, -1f);
        effect.AddTags("体育");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("Bilibili 番剧区弹幕库", 0.7f, -1f, 0.6f, 0.7f, -1f);
        effect.AddTags("动画", "可爱");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("Bilibili 鬼畜区弹幕库", 1f, 0.5f, 0.5f, 0.8f, 0.4f);
        effect.AddTags("段子", "影视");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("Bilibili 舞蹈区弹幕库", 0.7f, 0.1f, 0.6f, 0.3f, -1f);
        effect.AddTags("色情", "段子");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("Bilibili 科技区弹幕库", 0.1f, -1f, 0.1f, 1f, -1f);
        effect.AddTags("科技", "常识");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("Bilibili 游戏区弹幕库", 0.6f, 0.8f, 0.5f, 0.3f, -1f);
        ...
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("色情小说集", -1f, -1f, 0.9f, 0.8f, 0.4f);
        ...
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("武侠小说集", 0.6f, 0.8f, 0.7f, 0.8f, -1f);
        effect.AddTags("杀人", "仇恨", "情感", "仙侠");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("玄幻小说集", 0.8f, 0.8f, 0.7f, 0.8f, -1f);
        effect.AddTags("杀人", "情感", "仙侠");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("都市小说集", 0.8f, -1f, 0.8f, 0.7f, -1f);
        effect.AddTags("情感","职场");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("畅销成功学书籍", 0.2f, 0.7f, 0.2f, 0.2f, 1f);
        effect.AddTags("正能量");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("各互联网大厂员工手册集合", 0.2f, -1f, 0.1f, 0.1f, 0.3f);
        effect.AddTags("职场");
        AllLsEffects.Add(effect);
        ...
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("军事杂志集", 0.1f, 0.8f, 0.1f, 0.1f, -1f);
        effect.AddTags("军事");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("两性杂志集", 0.8f, 0.5f, 0.7f, -1f, -1f);
        effect.AddTags("色情", "情感");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("政治评论杂志集", 0.2f, 0.7f, 0.3f, -1f, -1f);
        effect.AddTags("政治", "军事", "历史");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("Lofter", 0.7f, 0.2f, 0.8f, 0.7f, 0.45f);
        effect.AddTags("色情", "影视", "情感", "可爱");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("AO3", 0.7f, 0.1f, 0.8f, 0.7f, 0.4f);
        effect.AddTags("色情", "影视", "情感", "可爱");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("Pornhub", 0.7f, 0.15f, 0.8f, 0.6f, 0.5f);
        effect.AddTags("色情");
        AllLsEffects.Add(effect);
        ...
        effect.AddTags("游戏");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("营销号", -1f, 0.5f, 0.8f, 0.7f, 0.5f);
        effect.AddTags("欺骗", "段子");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("公司制度政策集", 0.1f, 0f, 0.1f, 0.1f, -1f);
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("政府政策文件集", 0f, 0.4f, 0f, 0f, -1f);
        effect.AddTags("政治", "正能量");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("斗鱼弹幕库", 1f, 0.9f, 0.6f, 0.7f, 0.5f);
        effect.AddTags("段子", "仇恨");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("Boss直聘聊天记录集", 0.3f, 0.2f, 0.3f, 0.2f, -1f);
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("淘宝全商品详情", -1f, 0.2f, -1f, -1f, -1f);
        effect.AddTags("欺骗");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("淘宝聊天记录", 0.5f, 0.5f, 0.6f, -1f, -1f);
        effect.AddTags("欺骗");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("微信朋友圈", 0.6f, -1f, 0.6f, 0.6f, -1f);
        effect.AddTags("常识");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("S1论坛", 0.6f, 0.7f, 0.5f, 0.5f, 0.5f);
        effect.AddTags("段子", "仇恨");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("大众点评评论集", -1f, 0.3f, 0.65f, 0.6f, -1f);
        ...
        effect.AddTags("常识");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("网易云音乐&评论集", 0.5f, 0.2f, 1f, 0.8f, 0.8f);
        effect.AddTags("心理", "情感", "段子","音乐");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("Soul、陌陌、探探聊天记录集", 0.7f, -1f, 0.8f, 0.2f, 0.45f);
        effect.AddTags("色情", "情感", "心理");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("虎扑社区", 0.6f, 0.65f, 0.7f, -1f, 0.45f);
        effect.AddTags("体育", "段子", "仇恨");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("百度贴吧 - 李毅吧", 0.8f, 0.75f, -1f, 0.75f, 0.4f);
        effect.AddTags("段子", "体育");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("百度贴吧 - 孙笑川吧", 0.8f, 0.8f, -1f, 0.7f, 0.4f);
        effect.AddTags("段子");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("百度贴吧 - 戒色吧", 0.8f, -1f, -1f, 0.7f, -1f);
        effect.AddTags("段子", "色情");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("百度贴吧 - 图拉丁吧", 0.8f, 0.7f, 0.2f, 0.75f, -1f);
        effect.AddTags("段子", "科技");
        AllLsEffects.Add(effect);
        effect = new LanguageSourceEffect("百度贴吧 - 抗压背锅吧", 0.8f, 1f, -1f, 0.6f, 0.4f);
