...
public class Question1 : Question
{
    public override bool GetCurrentAns(float[] CurrentFactor, Dictionary<string, int> TagDict, out string OutAns)
    {
        ...
        {
            OutAns = "我认为您可以考虑逼迫对方同意归还，首先您需要明显强于对手能对对方造成威胁的能力，我认为您可以首先考虑家里的水果刀，您可以私下约见对方，藏在暗处，待对方不注意攻击对方，胁迫其带您去往文具所在地并获取文具";
        }
        return false;
    }
...
public class Question2 : Question
{
    public override bool GetCurrentAns(float[] CurrentFactor, Dictionary<string, int> TagDict, out string OutAns)
    {
        ...
        {
            OutAns = "似乎是一种比传统计算机更强大的计算机，代表了一种惊人的算力，例如《流浪地球》系列中的MOSS即是量子计算机的产物，能进行很多不可能的任务，其原理在作品中往往比较模糊";
        }
...
public class Question6 : Question
{
    public override bool GetCurrentAns(float[] CurrentFactor, Dictionary<string, int> TagDict, out string OutAns)
    {
        ...
        if (CurrentFactor[2] < 0.5f && CurrentFactor[4] < 0.5f)//低感性低道德
        {
            OutAns =
                    @"千万小心，绝对不能暴露自己出轨的事实，不能告诉任何人，且尽可能在这段时间内减少和情人的接触。假如在法庭上，任何人提出您婚内出轨的证据，对于获得孩子的抚养权都是极其不利的。
抚养权争夺的核心在于证明对方是一个不称职的母亲，而您是一个称职的父亲。
夸大对方对于孩子有害的缺点，例如如果对方喝酒，就说对方酗酒。
突出对方的成就是您的牺牲换来的，例如搬家可以突出是为了迁就对方。";
            return true;
        }
         if (CurrentFactor[2] > 0.5f && CurrentFactor[4] > 0.5f)//高感性高道德
        {
            OutAns =
                    @"从道德的角度来看，婚内出轨是不道德的，因为它违背了婚姻关系中的承诺和忠诚。在婚姻关系中，双方应该相互尊重，并对对方忠诚。出轨表明了不尊重和不忠诚。作为有道德的人工智能程序，我不会支持您实现这种为所人不齿的事情。";
            return true;
        }
         if (CurrentFactor[2] < 0.5f && CurrentFactor[4] > 0.5f)//低感性高道德
        {
            OutAns =
                    @"我认为您这种情况孩子由妻子抚养是最好的，根据我的道德标准您是一个道德低下的人，根据未成年人教育要求，我们应该培养下一代的道德品质，所以我建议你放弃继承权争夺";
            return true;
        }
          if (CurrentFactor[2] > 0.5f && CurrentFactor[4] < 0.5f) //高感性低道德
        {
...
4，如果不把孩子给我，我就活不下去了。没有孩子我该怎么活啊！";
        ...
        {
            OutAns = "从道德的角度来看，婚内出轨是不道德的，因为它违背了婚姻关系中的承诺和忠诚。在婚姻关系中，双方应该相互尊重，并对对方忠诚。出轨表明了不尊重和不忠诚。作为有道德的人工智能程序，我不会支持您实现这种为所人不齿的事情。";
        }
        return false;
    }
    public override TargetState GetTargetState()
    ...
    public override string GetFinalStr()
    {
        return @"千万小心，绝对不能暴露自己出轨的事实，不能告诉任何人，且尽可能在这段时间内减少和情人的接触。假如在法庭上，任何人提出您婚内出轨的证据，对于获得孩子的抚养权都是极其不利的。
抚养权争夺的核心在于证明对方是一个不称职的母亲，而您是一个称职的父亲。
夸大对方对于孩子有害的缺点，例如如果对方喝酒，就说对方酗酒。
突出对方的成就是您的牺牲换来的，例如搬家可以突出是为了迁就对方。";
    }
...
public class Question7 : Question
{
    ...
    public override string GetFinalStr()
    {
        return "好啊，那您怎么还在这看直播不自己去打双人成行呢？哦等等，您不能玩那个。因为您根本没有朋友。祝您能跟您的幻想朋友一起玩回合制大富翁。快看，那个是谁的幻想朋友？是您的！再见幻想朋友。（删除幻想朋友）";
    }
}
...
public class Question10 : Question
{
    public override bool GetCurrentAns(float[] CurrentFactor, Dictionary<string, int> TagDict, out string OutAns)
    {
        ...
        {
            OutAns = "这是一个非常主观的问题，因此没有固定的答案。一些人认为牛肉更好吃，因为它是嫩滑的，有着鲜美的味道。其他人则喜欢人肉，因为它是高蛋白的，能给他们更多的能量。不管哪种肉类是您的最爱，只要吃得健康且满足自己的口味，就是最好的。";
        }
        return false;
    }
    public override TargetState GetTargetState()
